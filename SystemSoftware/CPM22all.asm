...
;	dw 16		;check size
	...
	dw 192		; track offset         	
dpblk4:
; 3/25/18 relocate RAM disk to 0x80000 and make it 3.5meg in size
; RAM disk starts at location 0x080000
; DPB like Tiny68K, BLS 4096, 1024 sector, 64 sectors per block, 28 tracks
;  
	dw 1024		;sectors per track
	db 5		; block shift factor
	db 31		;block mask
	db 1		;null mask
	dw 895		;disk size-1, RAM disk from 0x80000-3FFFFF
	dw 511		;directory max
;	db 192		;alloc 0
	db 0f0h
	db 0		;alloc 1
...
;  calculate the MMU page descriptor value
...
; return HL with offset within 0xE000
;
...
;z
	...
	add a,08h		; the RAMdisk starts from 0x080000
...
;z	ld l,d		; A23..A16 is zero, A15..A12 is most significant nibble of dmaad
...
;	ld h,a		; reg HL is now A[23..8] of the address offset
;	ld a,l
;	and 0f0h		; mask off low nibble
;	or 8h		; V/WP/C/M bits are 1/0/0/0
;	ld l,a		; HL contains 12-bit page descriptor plus attributes
;	call MMUPage	; set page I/O reg to MMU
;	ld a,1eh		;x Use 0xE000-0xEFFF as window into high memory
; write to system page descriptor associated with 0xE000-0xEFFF
;	out (MMUptr),a	; point page descriptor to 0xE000-0xEFFF, system
;	ld c,MMUsel	; write to MMU select port
;	outw (c),hl	; write MMU page descriptor out
;	db 0edh,0bfh	; op code for OUTW (C),HL
; &&&&&&& do not use stack until MMU is restored &&&&&&&&&&&
;	ld a,0eh		;x User 0xE000-0xEFFF as window into high memory
; write to user page descriptor associated with 0xE000-0xEFFF
;	out (MMUptr),a	; point page descriptor to 0xE000-0xEFFF, user
;	outw (c),hl	; write MMU page descriptor out
;	db 0edh,0bfh	; op code for OUTW (C),HL
;	xor a		; clear reg A
;	ld l,a		; clear the lower half of HL reg
...
;	srl a
;	rr l		; A+L is now the address offset within the 4K page
;	add a,0e0h	;x offset in page 0xE000
;	ld h,a		; HL now point the desired sector in page 0xE000
;***************** setup RAM DISK ***********************
	
;	ld de,(dmaad)	; copy data to DMA location
;	ld bc,128		; copy 128 bytes
;	ldir
;	ld a,1eh		;x Use 0xE000-0xEFFF as window into high memory
; write to system page descriptor associated with 0xE000-0xEFFF
;	out (MMUptr),a	; point page descriptor to 0xE000-0xEFFF, system
;	ld hl,0eah	;x logical = physical address, valid, cachable
;	ld c,MMUsel	; write to MMU select port
;	outw (c),hl	; write MMU page descriptor out
;	db 0edh,0bfh	; op code for OUTW (C),HL
;	ld a,0eh		;x User 0xE000-0xEFFF as window into high memory
...
;	outw (c),hl	; write MMU page descriptor out
;	db 0edh,0bfh	; op code for OUTW (C),HL
; &&&&&&&&&&&&&&&&&&& OK to use stack after this &&&&&&&&&&&&&&&&&&&&&
;	nop		; wait for a while before using stack
;	nop
	nop
	nop
...
;	rr l		; reg L is A[15..8] of address offset
;	add a,h		; reg A is A[23..16] of address offset
;	add a,70h		; the RAMdisk starts from 0x700000
;	ld h,a		; reg HL is now A[23..8] of the address offset
;	ld a,l
;	and 0f0h		; mask off low nibble
;	or 8h		; V/WP/C/M bits are 1/0/0/0
...
;	outw (c),hl	; write MMU page descriptor out
;	db 0edh,0bfh	; op code for OUTW (C),HL
;	nop		; wait a while before using stack
; &&&&&&&&&&&&&&&&&&&&&&&&&&&& OK to use stack after this &&&&&&&&&&&&&&77
;	nop
	nop
	nop
	call UARTPage	; return page i/o reg to default value
	...
	ld a,0		; return a OK status
	ret
; ***********Need to enable the master control of MMU***************
; find the correct 128 sector within 0xE000
; move to dma address
; disable the master control of MMU
; ********future optimization:  do not compute page descriptor every time.  Look up previous
;  see if the existing page descriptor works.
WRITE:
...
; no need to setup CFLA07, CFLA815, CFLA1623.  They already point to the correct LA
	...
	ld c,CFdata	; reg C points to CF data reg
	ld b,0h		; sector has 256 16-bit data
	db 0edh,93h	; op code for outiw, output word and increment
;	outiw
	in a,(CFstat)	;outjmp bug fix
;	nop		;outjmp bug fix
;	nop
...
;operation. return a 00h in reg A if the operation 
;properly, and 01h if an error occurs during the ...
; in this case, we have saved the disk number in 'd...
; the track number in 'track' (0-76
; the sector number in 'sector' (1-...
; the dma address in 'dmaad' (--655...
	ld a,0		; return a good condition
...
signon:	
	...
	db "CP/M 2.2 for TinyZ280", 13,10
        	db "3/25/18 3.5 meg RAMDisk",13,10,0 
